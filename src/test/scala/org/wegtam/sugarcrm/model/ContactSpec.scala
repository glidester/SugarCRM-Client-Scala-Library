...
class ContactSpec extends FunSpec with Matchers {
  ...
  describe("Contact") {
    ...
    it("should read an existing contact") {
      ...
      def testDeserializer(json: String) = {
        val optCursor = for (json <- Parse.parseOption(json)) yield json.cursor
      ...
      testDeserializer(jsonContact1)
      testDeserializer(jsonContact2)
    }

    it("should convert the attributes to a NameValueList") {
      val contact = CRM_Contact(None, false, "Test User", "1", "2015-03-03 09:00:00",
    ...
    it("should be able to create, retrieve, and update") {
      ...
      val jsResult = getTestCRMConnection.setEntry("Contacts", Contact.buildNameValueList(contact))
      val newContact = Contact.parseJson(jsResult)

      newContact shouldBe 'isDefined

      val newDescription = "I have updated this contact"
      val alteredContact = newContact.get.asInstanceOf[CRM_Contact].copy(description = newDescription)

      val jsResult2 = getTestCRMConnection.setEntry("Contacts",Contact.buildNameValueList(alteredContact))

      val newAltContact = Contact.parseJson(jsResult2)

      newAltContact shouldBe 'isDefined
      newAltContact.get.description shouldEqual (newDescription)
  ...
  lazy val jsonContact2 = """{"id":"3ebfca42-9229-379d-6bdc-54d9d15e344b","entry_list":{"email2":{"name":"email2","value":"Test2@Test2.com"},"first_name":{"name":"first_name","value":"Unit"},"created_by":{"name":"created_by","value":"c7149e71-f7ba-f42e-0327-54eddd108ea8"},"description":{"name":"description","value":"A test"},"modified_user_id":{"name":"modified_user_id","value":"c7149e71-f7ba-f42e-0327-54eddd108ea8"},"email1":{"name":"email1","value":"test@test.com"},"date_modified":{"name":"date_modified","value":"2015-03-04 10:03:53"},"phone_home":{"name":"phone_home","value":"01783 456742"},"phone_mobile":{"name":"phone_mobile","value":"0777 7348432"},"last_name":{"name":"last_name","value":"Test_f64685d0-5987-40c5-a3e9-848b40a69ac1"},"salutation":{"name":"salutation","value":"Mr"},"do_not_call":{"name":"do_not_call","value":false},"date_entered":{"name":"date_entered","value":"2015-03-03 09:00:00"},"deleted":{"name":"deleted","value":0}}}"""
}
